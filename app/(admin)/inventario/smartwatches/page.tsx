...
const NOMBRE_UNIFICADO = 'Pulsera Inteligente Xiaomi 9 Active'

const KEYWORDS = ['pulsera', 'band', 'watch', 'smartwatch', 'reloj']

function esSmartwatch(name: string): boolean {
...
async function loadData(): Promise<{ rows: Row[]; error: string | null }> {
  ...
  try {
    ...
    const items = stockRes.rows.filter(r => esSmartwatch(r.display_name))
    let disponibles = 0
    let precio = 0
    ...
    for (const p of pedidos.filter(p => p.entregadoAt)) {
      for (const item of p.items) {
        ...
        if (SMARTWATCH_NAMES.some(n => lower.includes(n) || n.includes(lower.trim()))) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Inventario de smartwatches — ingresos desde pedidos, ventas y disponibles desde GOcelular.
      </p>
        ...
        <>
          ...
          {rows.length === 0 ? (
            <div className="bg-white border border-gray-200 rounded-xl p-16 text-center">
              <p className="text-gray-400 text-sm">Sin smartwatches en el inventario.</p>
            </div>
          ) : (
