...
export default async function CelularesPage() {
  ...
  const totalEnTransito = enTransito.reduce((s, p) => s + p.items.reduce((s2, i) => s2 + i.cantidad, 0), 0)
  ...
  const subpages = [
    ...
    { href: '/inventario/celulares/en-transito', label: `Pedidos en tránsito (${totalEnTransito})` },
  ]
