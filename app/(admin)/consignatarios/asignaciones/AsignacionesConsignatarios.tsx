...
export default function AsignacionesConsignatarios({ borradores, confirmados }: { borradores: Asignacion[]; confirmados: Asignacion[] }) {
                      ...
                      <div className="mt-3 bg-white border border-gray-200 rounded-xl p-5 space-y-4">
                        ...
                        <div>
                          ...
                          <input type="text" value={firmadoPor} onChange={(e) => setFirmadoPor(e.target.value)}
                            placeholder="Nombre de quien recibe" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                        </div>
                        <FirmaCanvas onSave={(base64) => setFirma(base64)} />
                        {firma && <p className="text-xs text-green-600 font-medium">Firma capturada</p>}
                        <div className="flex gap-3 justify-end">
                          <button type="button" onClick={() => { setConfirming(null); setFirma(null); setFirmadoPor('') }}
                            className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Cancelar</button>
                          <button type="button" onClick={() => handleConfirm(a.id)}
